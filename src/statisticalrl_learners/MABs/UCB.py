...
class UCB(BanditAgent):
    ...
    def __init__(self,nbArms,delta):
        self.delta = delta
        BanditAgent.__init__(self, nbArms, name="UCB")
    ...
    def update(self, arm, reward):
        ...
        self.indexes = [self.means[a] + sqrt(log(1/self.delta(self.time))/(2*self.nbDraws[a])) if self.nbDraws[a] > 0 else np.Inf for a in range(self.nA)]
